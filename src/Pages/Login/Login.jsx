...
const Login = () => {
  ...
  const visibilityToggle = () => {
    setPasswordVisibility((preState) =>
      preState === "password" ? "text" : "password"
    );
  };
  ...
  const fetchLogin = async () => {
    ...
    if (!password.match(validation.uppercaseLetters)) {
      setAuthValidation(
        "password should include at list one uppercase letter!"
      );
      return;
    }
    if (!password.match(validation.specialChars)) {
      setAuthValidation(
        "password should include at list one special character!"
      );
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        "https://digitalamazon.azurewebsites.net/api/User/LogIn",
        {
          method: "POST",
