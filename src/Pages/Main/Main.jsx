...
const Main = () => {
  ...
  const { categories, categoriesError } = useSelector(
    (state) => state.categories
  );
  const { offersData, offersError } = useSelector((state) => state.offers);
  const { suggestionsData, suggestionsError } = useSelector(
    (state) => state.suggestions
  );
  const { recommendedData, recommendedError } = useSelector(
        ...
        <div>
          <ul>
            {categories && categories.length ? (
              ...
              <h1 className="error">Categories could not Load...</h1>
            )}
          </ul>
        </div>
      </section>
        ...
        <div className="region-container">
          ...
          <div className="region-suppliers">
            <ul>
              {region.map((item) => (
                <li key={item.flag}>
                  <Region flag={item.flag} domain={item.domain} />
                </li>
              ))}
            </ul>
          </div>
