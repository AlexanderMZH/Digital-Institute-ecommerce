...
const ListItems = () => {
  const { categories, categoriesError, categoryId } = useSelector(
  ...
  const addToCart = async (itemId) => {
    ...
    try {
      await fetch(
        "https://digitalamazon.azurewebsites.net/api/cart/addincart",
        {
          method: "POST",
          headers: {
  ...
  const brandFilter = (name, event) => {
    ...
    if (event.target.checked) {
      fetch(
        `https://digitalamazon.azurewebsites.net/api/product/products?CategoryId=${queryParams.CategoryId}&PriceFrom=${queryParams.PriceFrom}&PriceTo=${queryParams.PriceTo}&BrandName=${name}`
      )
        .then((res) => res.json())
  ...
  const getFilteredProduct = async () => {
    try {
      const res = await fetch(
        `https://digitalamazon.azurewebsites.net/api/product/products?CategoryId=${queryParams.CategoryId}&PriceFrom=${queryParams.PriceFrom}&PriceTo=${queryParams.PriceTo}`
      );
            ...
            <div className="list-items-container-header">
              <div className="wide-width">
                <div className="item-capacity">
                  <div>
                    <span>{categoryItems.length} items in</span>
                  </div>
                  <div>
                    {categoryPath.map((item) => {
                      return <span key={item.id}>{item.name}</span>;
              ...
              <div className="mobile-width">
                <div
                  className="sort"
                  onClick={() => {
                    setSortToggle((preState) =>
                      preState === "" ? "sort-toggle" : ""
                    );
                  }}
                >
                  <div>
